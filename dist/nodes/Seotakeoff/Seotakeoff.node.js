...
class Seotakeoff {
    constructor() {
        ...
        this.methods = {
            loadOptions: {
                async getWebsites() {
                    const websites = await GenericFunctions_1.seotakeoffApiRequest.call(this, 'GET', '/websites');
                ...
                async getClusters() {
                    const clusters = await GenericFunctions_1.seotakeoffApiRequest.call(this, 'GET', '/clusters');
            ...
            try {
                ...
                if (resource === 'article') {
                    if (operation === 'generate') {
                        ...
                        responseData = await GenericFunctions_1.seotakeoffApiRequest.call(this, 'POST', '/articles/generate', body);
                    }
                    if (operation === 'get') {
                        const articleId = this.getNodeParameter('articleId', i);
                        responseData = await GenericFunctions_1.seotakeoffApiRequest.call(this, 'GET', `/articles/${articleId}`);
                    }
                    if (operation === 'getAll') {
                        const returnAll = this.getNodeParameter('returnAll', i);
                        if (returnAll) {
                            responseData = await GenericFunctions_1.seotakeoffApiRequest.call(this, 'GET', '/articles');
                        }
                        else {
                            const limit = this.getNodeParameter('limit', i);
                            responseData = await GenericFunctions_1.seotakeoffApiRequest.call(this, 'GET', '/articles', {}, { limit });
                        }
                    }
                    if (operation === 'search') {
                        const query = this.getNodeParameter('searchQuery', i);
                        responseData = await GenericFunctions_1.seotakeoffApiRequest.call(this, 'GET', '/articles/search', {}, { query });
                    }
                }
                if (resource === 'cluster') {
                    if (operation === 'create') {
                        const name = this.getNodeParameter('clusterName', i);
                        const seedKeywordsRaw = this.getNodeParameter('seedKeywords', i);
                        ...
                        responseData = await GenericFunctions_1.seotakeoffApiRequest.call(this, 'POST', '/clusters/create', body);
                    }
                    if (operation === 'getAll') {
                        responseData = await GenericFunctions_1.seotakeoffApiRequest.call(this, 'GET', '/clusters');
                    }
                    if (operation === 'search') {
                        const query = this.getNodeParameter('clusterSearchQuery', i);
                        responseData = await GenericFunctions_1.seotakeoffApiRequest.call(this, 'GET', '/clusters/search', {}, { query });
                    }
                }
